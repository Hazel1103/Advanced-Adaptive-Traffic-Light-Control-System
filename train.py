...
# Function to get the current phase index and duration for traffic lights
def get_traffic_light_phases():
    # Get a list of all traffic lights
    ...
    for tl in traffic_lights:
        ...
        # Store the index and duration in the dictionary
        phase_info[tl] = (current_phase_index, phase_duration)
...
def calculate_phase_duration(num_cars, min_duration=5, max_cars=15, max_duration=15):
    """
    Calculate the phase duration based on the number of cars.
    If num_cars is equal to or greater than max_cars, return max_duration.
    If num_cars is less, scale the duration proportionally between min_duration and max_duration.
    Ensure the returned value does not exceed min_duration and max_duration.
    """
    if num_cars >= max_cars:
        return max_duration
    # Scale the duration proportionally based on the number of cars
    calculated_duration = int(min_duration + ((max_duration - min_duration) / max_cars) * num_cars) + 1
    # Ensure the calculated duration is within the specified limits
    return int(max(min_duration, min(calculated_duration, max_duration)))

def send_data_with_flag(serial_port, baud_rate, junction_number):
    """Communicate with a device over serial port and return received data as integers."""
    START_FLAG = 'S'
    NUM_BYTES_TO_RECEIVE = 4
    received_data_int = []
    ...
    try:
        ...
        # Construct command as bytes
        command = bytes([ord(START_FLAG)])
        ser.write(command)
        ...
        received_data_int = [int(byte) for byte in received_data]
...
def send_lane_with_flag(serial_port, baud_rate, duration,lane):
    """Communicate with a device over serial port and return received data as integers."""
    START_FLAG = 'A'
    try:
        # Initialize serial connection
        ...
        # Construct command as bytes
        command = bytes([ord(START_FLAG),lane,duration])
        ser.write(command)
    except serial.SerialException as e:
        print(f"Serial port error: {e}")
    except Exception as e:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    if(junction_number == 0):
                        state_= send_data_with_flag("COM6",115200,junction_number)
                    if(junction_number == 1):
                        state_= send_data_with_flag("COM15",115200,junction_number)
                    print(f"The state for Junction {junction_number} = {state_}")
                    # state_ = list(vehicles_per_lane.values()) 
                    state = prev_vehicles_per_lane[junction_number]
                    # state = ask_for_state(junction_id)
                    ...
                    # print(f"number of cars in selected lane {lane} = {state_[lane]}")
                    time.sleep(0.1)
                    
                    calculated_duaration = calculate_phase_duration(state_[lane],5,15,25)
                    if(junction_number == 0):
                        send_lane_with_flag("COM6",115200,calculated_duaration,lane)
                    if(junction_number == 1):
                        send_lane_with_flag("COM15",115200,calculated_duaration,lane)
                    print(f"thees is lane for junction  {junction_number} ==> {lane}")
                    phaseDuration(junction, 6, select_lane[lane][0])
                    phaseDuration(junction, calculated_duaration, select_lane[lane][1])
                    # Print the selected lane and phase duration with the junction ID
                    # print(f"Junction: {junction}, Selected lane: {lane}")
                    if ard:
                        ph = str(traci.trafficlight.getPhase("0"))
                        value = write_read(ph)

                    traffic_lights_time[junction] = calculated_duaration
                    if train:
                        brain.learn(junction_number)
                else:
                    traffic_lights_time[junction] -= 1
            print(f"time = {traffic_lights_time}")
            step += 1
