...
def combine_list_items(input_list):
    # Check if the length of the list is greater than 4
    if len(input_list) > 4:
        # Initialize the new list
        new_list = []
        # Iterate over the input list in steps of 2
        for i in range(0, len(input_list), 2):
            # Sum two items and add to the new list
            if i + 1 < len(input_list):
                new_list.append(input_list[i] + input_list[i + 1])
            else:
                new_list.append(input_list[i])
        
        # Ensure the new list has exactly 4 items
        while len(new_list) > 4:
            new_list = new_list[:4]
        return new_list
    else:
        # If the length of the list is not greater than 4, return the original list
        return input_list

def get_vehicle_numbers(lanes):
    vehicle_per_lane = dict()
    
    # Populate the initial dictionary with vehicle counts per lane
    for l in lanes:
    ...
    # If the initial dictionary has more than 4 items, combine pairs until only 4 items remain
    while len(vehicle_per_lane) > 4:
        new_vehicle_per_lane = {}
        keys = list(vehicle_per_lane.keys())

        # Combine every two items
        for i in range(0, len(keys), 2):
            if i + 1 < len(keys):
                new_key = f"{keys[i]}+{keys[i+1]}"
                new_value = vehicle_per_lane[keys[i]] + vehicle_per_lane[keys[i+1]]
            else:
                new_key = keys[i]
                new_value = vehicle_per_lane[keys[i]]

            new_vehicle_per_lane[new_key] = new_value

        vehicle_per_lane = new_vehicle_per_lane

    return vehicle_per_lane 

def get_traffic_light_phases(junction_id):
    """
    Retrieves the traffic light phases for a specified junction.

    Args:
        junction_id (str): The ID of the junction.

    Returns:
        list: A list of traffic light phase states.
    """
    phases = []
    tls_logics = traci.trafficlight.getAllProgramLogics(junction_id)
    for logic in tls_logics:
...
def calculate_phase_duration(num_cars, min_duration=5, max_cars=25, max_duration=40):
    """
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    more_than_four = list(vehicles_per_lane.values())
                    state_ = list(vehicles_per_lane.values())                        
                    state = prev_vehicles_per_lane[junction_number]
                    # state =  combine_list_items(state_uncheck)
                    ...
                    select_lane_action = get_traffic_light_phases(junction)
                    print(f"state = {state_}")
                    print(f"l-ane = {lane}")
                    calculated_duaration = calculate_phase_duration(state_[lane],5,30,45)
                    phaseDuration(junction, calculated_duaration , select_lane_action[lane])
